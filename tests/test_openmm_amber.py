...
#                                                   Density    =         0.6634
        ...
        self.assertAlmostEqual(energies['nonbonded'], 1200.11736916, delta=0.005)
        ...
        # correction (but that is kept on for this tests -- there is no direct
        ...
        self.assertAlmostEqual(energies['angle'], 0.9616, 4)
        self.assertAlmostEqual(energies['dihedral'], -5.4917, 4)
        self.assertAlmostEqual(energies['nonbonded'], 1168.06630486, delta=0.005)
        # Now make sure that if we use a switching function, that gets
        # propagated to the CustomNonbondedForce
        system = parm.createSystem(nonbondedMethod=app.PME, nonbondedCutoff=8*u.angstroms, switchDistance=6*u.angstroms)
        n = 0
        for force in system.getForces():
            if isinstance(force, mm.NonbondedForce) or isinstance(force, mm.CustomNonbondedForce):
                self.assertTrue(force.getUseSwitchingFunction())
                self.assertAlmostEqualQuantities(force.getSwitchingDistance(), 6*u.angstroms)
                n += 1
    ...
    def test_nbfix_exceptions(self):
        ...
        e = context.getState(getEnergy=True).getPotentialEnergy().value_in_unit(u.kilocalories_per_mole)
    ...
    def test_1012(self):
        ...
        self.assertEqual(parm.combining_rule, 'lorentz')
        system = parm.createSystem(nonbondedMethod=app.PME, nonbondedCutoff=8*u.angstroms)
        for force in system.getForces():
            if isinstance(force, mm.CustomNonbondedForce):
                self.assertTrue(force.getUseLongRangeCorrection())
        ...
        # Now tests with the 12-6-4 potential. sander does not implement this
        ...
        # never considered. This tests is here just to ensure coverage and
    ...
    def test_interface_pbc(self):
        ...
        for f in system.getForces():
            ...
            if isinstance(f, mm.NonbondedForce):
                ...
                self.assertEqual(f.getNumBonds(), parm.ptr('nbona')+parm.ptr('nbonh'))
            elif isinstance(f, mm.HarmonicAngleForce):
                self.assertEqual(f.getNumAngles(), parm.ptr('ntheth')+parm.ptr('ntheta'))
            elif isinstance(f, mm.PeriodicTorsionForce):
                self.assertEqual(f.getNumTorsions(), parm.ptr('nphih')+parm.ptr('nphia'))
        ...
        for f in system.getForces():
            ...
            if isinstance(f, mm.NonbondedForce):
                ...
                self.assertEqual(f.getNumAngles(), parm.ptr('ntheta')+parm.ptr('ntheth'))
            elif isinstance(f, mm.PeriodicTorsionForce):
                self.assertEqual(f.getNumTorsions(), parm.ptr('nphih')+parm.ptr('nphia'))
        ...
        for f in system.getForces():
            ...
            if isinstance(f, mm.NonbondedForce):
                ...
                self.assertEqual(f.getNumTorsions(), parm.ptr('nphih')+parm.ptr('nphia'))
        self.assertTrue(has_cmmotion)
        self.assertEqual(system.getNumConstraints(), parm.ptr('nbonh')+parm.ptr('nbona'))
        system = parm.createSystem(nonbondedMethod=app.CutoffPeriodic,
                                   nonbondedCutoff=20.0*u.angstroms,
                                   constraints=app.HBonds, rigidWater=True,
                                   flexibleConstraints=False,
        ...
        self.assertEqual(system.getNumConstraints(), 3*sum([r.name == 'WAT' for r in parm.residues]))
        # Trap some illegal options
        self.assertRaises(ValueError, lambda: parm.createSystem(nonbondedMethod=0))
    ...
    def test_dihedral_splitting(self):
        ...
        self.assertEqual(parm.omm_dihedral_force(split=False).getNumTorsions(), len(parm.dihedrals))
    ...
    def test_interface_no_pbc(self):
        ...
        for f in system.getForces():
            if isinstance(f, (mm.NonbondedForce, mm.CustomNonbondedForce)):
                self.assertEqual(f.getNonbondedMethod(), 0)
        system = parm.createSystem(nonbondedMethod=app.NoCutoff, implicitSolvent=app.OBC1, implicitSolventKappa=0.8)
        for f in system.getForces():
            if isinstance(f, (mm.NonbondedForce, mm.CustomNonbondedForce)):
                self.assertEqual(f.getNonbondedMethod(), 0)
        ...
        for f in system.getForces():
            if isinstance(f, (mm.NonbondedForce, mm.CustomNonbondedForce)):
                self.assertEqual(f.getNonbondedMethod(), 0)
        system = parm.createSystem(nonbondedMethod=app.NoCutoff, implicitSolvent=app.OBC2, implicitSolventKappa=0.8)
        for f in system.getForces():
            if isinstance(f, (mm.NonbondedForce, mm.CustomNonbondedForce)):
                self.assertEqual(f.getNonbondedMethod(), 0)
        ...
        system = parm.createSystem(nonbondedMethod=app.NoCutoff,
                                   ...
                                   implicitSolventSaltConc=0.1*u.molar,
                                   useSASA=True)
        frc = parm.omm_gbsa_force(app.GBn2 ,nonbondedCutoff=1.5, nonbondedMethod=app.CutoffNonPeriodic)
        self.assertEqual(frc.getCutoffDistance(), 1.5*u.nanometers)
        frc = parm.omm_gbsa_force(app.GBn2 ,nonbondedCutoff=1.5, nonbondedMethod=app.CutoffPeriodic)
        self.assertEqual(frc.getCutoffDistance(), 1.5*u.nanometers)
        ...
        self.assertRaises(ValueError, lambda: parm.createSystem(nonbondedMethod=app.PME))
        self.assertRaises(ValueError, lambda: parm.createSystem(nonbondedMethod=app.CutoffPeriodic))
        self.assertRaises(ValueError, lambda: parm.createSystem(nonbondedMethod=app.Ewald))
        self.assertRaises(ValueError, lambda: parm.createSystem(nonbondedMethod='cat'))
        self.assertRaises(ValueError, lambda: parm.createSystem(implicitSolvent='No model'))
